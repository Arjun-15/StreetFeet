...
export const forgetPassword = async (req, res, next) => {
  try {
    ...
    return next(new ErrorHandler(500, "Password reset failed"));
  }
};

export const resetUserPassword = async (req, res, next) => {
  try {
    // Hash the token sent via URL
    ...
    if (!user) {
      return next(new ErrorHandler(400, "Reset token is invalid or has expired."));
    }
...
export const updateUserProfileAndRole = async (req, res, next) => {
  ...
  try {
    const updatedUserDetails = await updateUserRoleAndProfileRepo(req.params.id, { name, email, role });
